...
**Risks addressed:** R-001, R-005, R-008, R-009
...
`BUILD_UNVALIDATED_ENVIRONMENT`. Diagnostics name `path`, expected type/value,
observed type/value, and the 64-KiB/128-row bound as applicable.
...
| BUILD-FAIL-008 | Reference JSON/ID/predicate is invalid, or package identity exceeds bounds/contains an unsupported entry. [R-005, R-008, R-009] | Canonical contract validation or package-tree walk before support/release acceptance. | Section 6.3 `BUILD_REFENV_*` code or `BUILD_PACKAGE_IDENTITY_INVALID`; path/operator/resource, limit, expected/observed. | No validated support state or accepted package identity; bounded temporary buffers released. | Correct/regenerate from declared inputs; never relabel mismatched bytes. | BUILD-FR-010, -011 / BUILD-TEST-010, -011. |
...
| BUILD-FR-010, BUILD-NFR-004 | BUILD-TEST-010 | Schema/integration/golden/resource | Fixed canonical example; reference plus schema/operator/ID/noncanonical and each observed-fact mismatch fixture; maximum-valid, 64-MiB+1 allocation, and synthetic 10-second+1-tick deadline fixtures. | Fixed ID matches; reference has nine passes within 64 MiB/10 seconds; malformed, memory-over, and deadline-over contracts return exact typed errors; each fact mismatch is unvalidated with exact path/outcome. | `reference-environment-report.json`, canonical-ID and resource goldens. |
| BUILD-FR-011 | BUILD-TEST-011 | End-to-end/resource/identity | Two clean packages; mtime, mode, path, symlink, special-file, 100,001-entry, 16-GiB-bound, 256-MiB+1 allocation, and synthetic 1,200-second+1-tick walk fixtures. | Required contents and both subject digests match across clean builds; mtime is excluded; included metadata/content changes only the specified identity; invalid, memory-over, or deadline-over tree rejects before identity acceptance while the maximum valid tree remains within 256 MiB RSS/1,200 seconds. | `package-provenance.json`, `package-identity-matrix.json`. |
