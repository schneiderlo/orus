...
| BUILD-FR-010, BUILD-NFR-004 | BUILD-TEST-010 | Schema/integration/golden | Fixed canonical example; reference plus schema/operator/ID/noncanonical and each observed-fact mismatch fixture. | Fixed ID matches; reference has nine passes; malformed contracts return exact typed errors; each fact mismatch is unvalidated with exact path/outcome. | `reference-environment-report.json`, canonical-ID golden. |
| BUILD-FR-011 | BUILD-TEST-011 | End-to-end/resource/identity | Two clean packages; mtime, mode, path, symlink, special-file, 100,001-entry, and 16-GiB-bound fixtures. | Required contents and both subject digests match across clean builds; mtime is excluded; included metadata/content changes only the specified identity; invalid/over-limit tree rejects. | `package-provenance.json`, `package-identity-matrix.json`. |
