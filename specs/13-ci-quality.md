...
| CI-FR-006 | GitHub-hosted benchmark jobs shall emit complete results with `authority=advisory` and shall not fail solely because of measured delta. | Benchmark schema validation, tool failure, or typed-report/retained-log first-over resource failure blocks its own functional job; a simulated 10% regression with otherwise valid shared-runner provenance remains advisory and cannot fail `M0-CI-GATE-v1`; raw samples upload. | CI-TEST-006 authority and simulated-regression workflow test; CI-TEST-011 advisory evidence-limit fixtures. | SM-12, D-010, R-002, R-008, R-201. |
...
|---|---:|---|
| `M0-CI-CHECK-RESULT-v1`, gate, or other typed report | 8 MiB/document | Do not truncate a schema. Replace it with one <=64-KiB `M0-CI-EVIDENCE-LIMIT-v1` containing run/attempt/check, class, original byte count, limit, full-stream SHA-256 if safely available, `truncated=false`, and `code=CI_EVIDENCE_LIMIT`; the terminal job state is `failed` for both blocking and advisory jobs. An advisory benchmark report failure is functional and does not make its measured delta blocking. |
...
| Retained normalized job log | 32 MiB | Retain first 16 MiB and last 16 MiB at line/scalar boundaries plus omitted-byte count and full-stream SHA-256; set `truncated=true` and set terminal job state `failed` for both blocking and advisory jobs even if the command exited 0. An advisory benchmark log failure is functional and does not make its measured delta blocking. Raw provider log is not release evidence. |
...
| CI-FAIL-007 | Missing/stale/malformed/cancelled required evidence, a Section 6.3 numeric evidence limit, or known provider timeout. [R-008, R-201] | Producer resource guard, authenticated provider terminal event, and final aggregation. | A typed report above 8 MiB or retained log above 32 MiB is exactly `CI_EVIDENCE_LIMIT` with job state `failed` regardless of blocking/advisory authority; this functional failure does not make an advisory benchmark's measured delta blocking. A known provider timeout is also exactly `CI_EVIDENCE_LIMIT`/`failed`. A per-job bundle above 64 MiB is `CI_EVIDENCE_LIMIT`/`failed` for a blocking job and exactly `CI_EVIDENCE_LIMIT`/`incomplete` for an advisory job. Missing/cancelled/stale/malformed evidence is `CI_GATE_INCOMPLETE`. Diagnostics include expected check/class, byte/count/time limit, observed state/identity, and run/attempt. | No blocking pass or advisory functional pass; bounded partial/limit evidence retained; timed-out process group terminated and rejected staged upload removed. | Fix producer output or retry missing/cancelled jobs; never reinterpret report/log truncation, an advisory incomplete bundle, or timeout as pass. | CI-FR-006, -010, -011 / CI-TEST-006, -010, -011. |
...
| CI-FR-006, CI-FR-011 | CI-TEST-011 | Integration/resource/timeout | Forced failure; blocking and advisory 8-MiB typed reports, 32-MiB retained logs, and 64-MiB bundles; 4-KiB diagnostic, 1,000-finding, and job wall-time fixtures at exactly 120 minutes and 120 minutes + 1 nanosecond. Each numeric fixture has a first-over value of one byte or one nanosecond; include UTF-8 split, owned-process cleanup, and secret fixtures. | Exact-bound artifacts and the exactly-120-minute completion pass structurally. For both blocking and advisory authority, an 8-MiB + 1-byte report and 32-MiB + 1-byte log emit exactly `CI_EVIDENCE_LIMIT`/`failed`; the advisory cases are functional failures and do not promote a measured delta. A blocking 64-MiB + 1-byte bundle emits `CI_EVIDENCE_LIMIT`/`failed`; an advisory 64-MiB + 1-byte bundle emits `CI_EVIDENCE_LIMIT`/`incomplete`; the first-over timeout terminates the group and emits/synthesizes exactly `CI_EVIDENCE_LIMIT`/`failed`. Every other evidence over-limit follows its exact reject/truncate rule; head/tail/count/digest are exact and no secret is retained. | `failure-evidence-report.json`, `ci-evidence-limit-matrix.json`. |
