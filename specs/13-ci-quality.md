...
**Risks addressed:** R-001, R-002, R-004, R-005, R-007, R-008
...
| CI-FR-002 | CI shall execute all Charter canonical commands through the pinned Nix environment and Bazel graph. | Each canonical command has one job/step ID; all exit 0 on reference fixture; logs prove Nix+Bazel delegation and expected toolchain; no CMake/host fallback. | CI-TEST-002 workflow-to-command reconciliation and clean run. | SM-01, DOD-03, R-001. |
| CI-FR-003 | `M0-CI-APPLICABILITY-v1` shall list every M0 Bazel target or finite target group against every M0 check. | Bazel query result and manifest expand to the same unique label set; each cell is `required` or `not_applicable` with a stable scoped rationale; missing, duplicate, unknown, or silently skipped cells fail. | CI-TEST-003 manifest schema/query reconciliation and mutation fixtures. | DOD-04, R-007. |
...
| CI-FR-007 | Formatting, lint, static checks, and compiler warnings shall use zero non-allowlisted warnings. | Every produced warning has stable tool/rule ID and target/config scope; unmatched warning count is zero; allowlist entry requires exact scope, owner, rationale, and review milestone; broad wildcard or expired entry fails. | CI-TEST-007 warning reconciliation and injected-warning fixtures. | DOD-04. |
| CI-FR-008 | Workflow actions and external build inputs shall be immutable and least-privileged. | Third-party actions are pinned to full commit SHAs; workflow/job permissions are explicit with default read-only contents and no write/id-token/package permission unless a separately reviewed release job requires it; pull-request jobs receive no protected secrets. | CI-TEST-008 workflow security static analysis and untrusted-PR simulation. | C-11, R-005. |
...
| CI-FR-011 | Failure paths shall retain bounded machine-readable evidence and human-actionable diagnostics. | Each job uploads its typed report even when tests fail, subject to size limits; log points to job/check/config/target and artifact name; secrets and unbounded target output are absent. | CI-TEST-011 forced-failure evidence test. | DOD-08, DOD-10. |
...
| CI-NFR-006 | Secret exposure: 0 protected secret values in pull-request environment, action inputs, caches, logs, or artifacts. | Untrusted-fork simulation with canary values and full workflow/artifact scan. | Canary count zero outside protected harness; no privileged permission available. | CI-TEST-008/009 security report. |
...
The committed manifest contains `schema`, source revision policy, check
...
incomplete. Logs/artifacts are bounded by job configuration; over-limit
reports truncate with `truncated=true` and preserve the summary/failure
identity.

### 6.4 Design choice: job topology
...
| CI-FAIL-006 | Unpinned action, excessive permission, PR secret/cache exposure. [R-005] | Workflow static/security validation before affected job. | `CI_SECURITY_POLICY_VIOLATION`; workflow/job, action/permission/capability. | Workflow/gate fails; no secret use. | Pin/reduce/isolate and rerun. | CI-FR-008, -009 / CI-TEST-008, -009. |
| CI-FAIL-007 | Missing/stale/malformed/cancelled required evidence. | Final aggregation. | `CI_GATE_INCOMPLETE`; expected check, observed state/identity, run/attempt. | No passed gate; partial evidence retained. | Retry missing jobs or whole run. | CI-FR-010, -011 / CI-TEST-010. |
...
ID, build configuration, target label, and artifact SHA-256 digest. M0 has no trace,
...
| CI-FR-002 | CI-TEST-002 | End-to-end/analysis | Reference clean run and workflow/Charter command inventory. | 100% command mapping/success; expected Nix/toolchain; no bypass. | `canonical-command-reconciliation.json`. |
...
| CI-FR-008, CI-NFR-006 | CI-TEST-008 | Security/static | Workflow AST and untrusted PR with canary secret. | Full SHA pins; least privilege; zero canary exposure. | `workflow-security.sarif`. |
...
| CI-FR-011 | CI-TEST-011 | Integration/resource | Forced job failures and report-size boundary. | Typed evidence uploads; bounded/truncated marker correct; no secret. | `failure-evidence-report.json`. |
