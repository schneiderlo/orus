...
| CI-FAIL-007 | Missing/stale/malformed/cancelled or numerically over-limit required evidence, including known provider timeout. [R-008, R-201] | Producer resource guard, authenticated provider terminal event, and final aggregation. | Numeric evidence/log/bundle excess or provider timeout: exactly `CI_EVIDENCE_LIMIT`, job state `failed`; missing/cancelled/stale/malformed evidence: `CI_GATE_INCOMPLETE`; expected check/class, byte/count/time limit, observed state/identity, run/attempt. | No passed gate; bounded partial/limit evidence retained; timed-out process group terminated and staged upload removed. | Fix producer output or retry missing/cancelled jobs; never reinterpret truncation or timeout as pass. | CI-FR-010, -011 / CI-TEST-010, -011. |
...
| CI-FR-011 | CI-TEST-011 | Integration/resource/timeout | Forced failure; 4-KiB diagnostic, 1,000-finding, 8-MiB report, 32-MiB log, 64-MiB bundle, and job wall-time fixtures at exactly 120 minutes and 120 minutes + 1 nanosecond; UTF-8 split, owned-process cleanup, and secret fixtures. | Exact-bound artifacts and the exactly-120-minute completion pass structurally; every evidence over-limit follows its exact reject/truncate rule; the first-over timeout terminates the group and emits/synthesizes exactly `CI_EVIDENCE_LIMIT` with terminal job state `failed`; head/tail/count/digest exact; zero secret. | `failure-evidence-report.json`, `ci-evidence-limit-matrix.json`. |
