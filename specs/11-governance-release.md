...
**Risks addressed:** R-004, R-005, R-008, R-010, R-201, R-202
...
| GOV-FR-004 | Every direct build/runtime dependency shall have one schema-valid, approved `M0-DEPENDENCY-ADMISSION-v1` record before it enters the lock graph. | Every required field, enum, bound, pin grammar/digest relationship, and other relationship in Section 6.1.1 validates within 64 MiB RSS and 30 seconds. Exactly 64 MiB and 30 seconds are permitted; the first attempted byte or nanosecond over either bound returns `GOV_RESOURCE_LIMIT`. `license_state=unknown_pending_legal` is represented only with `spdx_expression=null`, `status=blocked`, and a release-blocking notice disposition; it can never be admitted by default. Missing, denied, ambiguous, over-resource, or relationship-invalid records fail reconciliation with their exact Section 6.1.5 error. | GOV-TEST-004 admission-schema, pin-relationship, exact-bound/first-over resource, valid-example, unknown-license, and unrecorded-dependency fixtures. | G-03, R-005, R-201. |
...
| GOV-FR-006 | Each release package shall include a canonical SPDX 2.3 JSON SBOM bound to the packaged `orus` executable and its resolved graph. | The completed SBOM and external `M0-SBOM-CONTRACT-v1` descriptor satisfy the complete array/generated-field normalization in Section 6.1.2 within 256 MiB RSS and 120 seconds. Exactly 256 MiB and 120 seconds are permitted; the first attempted byte or nanosecond over either bound returns `GOV_RESOURCE_LIMIT` and creates no descriptor. The SBOM names `orus_executable_sha256`, includes every reconciled component/license/relationship, and contains neither its own digest nor `package_tree_sha256`; the external descriptor computes `sbom_sha256` over completed canonical SBOM bytes. Its generator is pinned, and mutable interpretation references are forbidden. | GOV-TEST-006 SPDX 2.3 schema, every-array normalization, every-generated-field, external-descriptor, exact-bound/first-over resource, generator-pin, graph, and artifact-binding tests. | G-03, SM-10, R-005, R-008, R-201; resolved `q-0004` and `q-0011`. |
...
| GOV-FR-010 | Publication shall be fail-closed and bound to one validated evidence bundle through the acyclic gate order in Section 6.1.3. | Any failed/missing validation, mismatched subject digest, unresolved dependency/license, unapproved protected decision, non-allowlisted claim, incomplete final secret scan, unscanned/unbound raw-string metadata, raw path/source value in the excluded control pair, or gate-order cycle produces non-zero status and no marker. Success freezes and scans the pre-approval manifest plus the spec-`16` pre-scan metadata document, validates the final manifest's metadata digest and digest-only displays, then creates one immutable external `M0-RELEASE-APPROVAL-v1` marker last, binding release ID, all three subject digests, the evidence manifest's `evidence_object_sha256`, and final secret-scan manifest/report digests. | GOV-TEST-010 end-to-end gate with one fault fixture per validator, subject substitution, metadata binding/redaction, and dependency-DAG/self-output fixtures. | DOD-01, DOD-06, R-004, R-005, R-008, R-202. |
...
3. enumerate and freeze spec `16`'s raw path/source metadata document, including
   its non-digest self tuple, then scan it as an ordinary `release_evidence`
   entry with this manifest and every referenced evidence byte; exclude only
   the current scan's own manifest/report and the not-yet-created marker;
4. validate the current scan's fixed schemas, one-to-one metadata binding,
   digest-only final-pair displays, completeness, redaction, input digests, and
   passing status; and
...
`GOV_RELATIONSHIP_INVALID`, `GOV_LICENSE_UNRESOLVED`, or
`GOV_RESOURCE_LIMIT`. `GOV_FIELD_BOUND` identifies a document field,
collection, or encoded-value bound. `GOV_RESOURCE_LIMIT` identifies only a
validator peak-RSS or wall-time excess and sets `field_path` to the owning
contract root, `expected` to `peak_rss_bytes` or `wall_time_ns`, `limit` to the
exact numeric ceiling, and `observed` to the bounded decimal
observed/requested value. Validation stops before approval side effects but may
return at most 1,000 sorted errors with a total count and `truncated=true` in
...
2. **Release candidate validation (GOV-FR-001, -002, -008 through -012).**
   ...
   `preapproval_validated`; the final secret scan covers that manifest, all
   variable evidence, and the frozen raw-string metadata document; its excluded
   manifest binds the scanned metadata and retains only digest displays; only
   then does the gate atomically create the external approval marker bound to
   the manifest and scan control pair.
...
| GOV-FAIL-007 | Release/secret-scan dependency graph refers to the current scan control pair/final marker before those bytes exist, or the pre-scan metadata is unscanned/unbound or permits a raw final-pair name. [R-005, R-202] | Dependency-DAG and final-pair metadata/redaction validation before marker. | `GOV_RELATIONSHIP_INVALID` for a forbidden edge; spec-`16` `SEC_SECRET_SCAN_INCOMPLETE` for metadata/redaction failure; source object/type, hashed locator, dependency edge or mismatch class, gate step. | Candidate rejected; no scan success or marker; frozen variable evidence and safe redacted failure pair remain diagnostic. | Remove the cyclic edge or regenerate from complete frozen metadata, then rerun the scan. | GOV-FR-008, -010 / GOV-TEST-008, -010. |
| GOV-FAIL-008 | A dependency-admission, SBOM, or release-evidence validator exceeds its owning RSS or wall-time ceiling. [R-201] | Validator resource guard before approval, descriptor creation, final scan, or marker. | Exactly `GOV_RESOURCE_LIMIT`; contract root, one of `peak_rss_bytes` or `wall_time_ns`, exact limit, observed/requested value. | Candidate/descriptor rejected; no approval marker; bounded temporary state released. | Retry with a bounded valid input or correct the validator; a larger limit requires a reviewed contract change. | GOV-FR-004, -006, -008 / GOV-TEST-004, -006, -008. |
...
| GOV-FR-004 | GOV-TEST-004 | Schema/negative/resource | Section 6.1.1 valid example; every pin grammar/digest relation; one missing/type/enum/bound/relationship negative per field; unrecorded, denied, unknown-license; exact-bound fixtures at 64 MiB peak RSS and 30 seconds; first-over fixtures at 64 MiB + 1 byte and 30 seconds + 1 nanosecond. | Both exact-bound fixtures pass; each first-over fixture emits exactly `GOV_RESOURCE_LIMIT`; every schema/content mutation emits its exact other Section 6.1.5 code; unknown license is blocked, never admitted. | `dependency-admission.json`. |
...
| GOV-FR-006, GOV-NFR-006 | GOV-TEST-006 | Schema/integration/reproducibility/resource | Section 6.1.2 example; two generated SPDX documents; every admitted SPDX array in two producer orders; every synthesized value; wrong format/generator/clock/random/ID/canonicalization/graph/executable, duplicate-array, self-digest, oversize/count/depth; exact-bound fixtures at 256 MiB peak RSS and 120 seconds; first-over fixtures at 256 MiB + 1 byte and 120 seconds + 1 nanosecond. | Valid outputs/descriptor are byte-identical with equal external `sbom_sha256` at either exact resource bound; every array is sorted by canonical element bytes, generated values equal formulas, no self/package digest exists, schema/content negatives fail with their exact code, and both first-over fixtures return exactly `GOV_RESOURCE_LIMIT` without a descriptor. | `orus.spdx.json`, `sbom-descriptor.json`, `sbom-validation.json`, `sbom-reproducibility.json`. |
...
| GOV-FR-008, GOV-NFR-004 | GOV-TEST-008 | End-to-end/schema/security/resource | Section 6.1.3 assembled example, complete 12-type/12-validator/3-role bundle, missing/duplicate type, producer identity/version, approval role/count, one-byte/included-metadata mutation, cross-subject substitution; exact-bound fixtures at 256 MiB peak RSS and 120 seconds; first-over fixtures at 256 MiB + 1 byte and 120 seconds + 1 nanosecond. | Complete bundle reaches `preapproval_validated` at either exact resource bound; each inventory/mutation/substitution error is detected with its exact code; both first-over fixtures return exactly `GOV_RESOURCE_LIMIT`; manifest has no self/current-scan/final-marker reference. | `release-evidence-v1.json`, inventory/resource and subject tamper reports. |
...
| GOV-FR-010 | GOV-TEST-010 | End-to-end/concurrency/cancellation/DAG/security | Valid candidate, fault per validator, subject substitution, missing/digest-drifted pre-scan metadata, raw/unmatched final-pair path/source fields, current-scan/final-marker cyclic-edge fixtures, concurrent attempts, and cancellation before commit. | Dependency trace is exactly evidence -> frozen release manifest/raw-string metadata -> complete scan -> digest/redaction-only final control pair -> marker; metadata digest/mapping is exact; only a complete passing acyclic candidate gets one marker and all other paths get none. | `release-gate-report.json`, `release-gate-order.json`. |
