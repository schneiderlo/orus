...
| [11-governance-release](specs/11-governance-release.md) | Draft | MIT license packaging with exact approved notice `Copyright (c) 2026 Loic Schneider`; outside-contribution closure; concrete dependency/SBOM/release/claims schemas including blocking unknown-license state; external canonical SPDX 2.3 JSON SBOM identity; release provenance; repository/agent policy; and rollback/compatibility claim rules. | `10` (Draft; Ready prerequisite); Foundation (Ready): D-002/D-009/D-016. |
| [12-cli-diagnostics](specs/12-cli-diagnostics.md) | Draft | Minimal `orus --version` and `orus doctor` commands emitting exactly one bounded JSON document per invocation, including usage/contract/renderer failures; exact reference-fact paths, exit codes, diagnostics, and false-claim prevention. No help/format/prose or record/replay command surface. | `10`, `11` (Draft; Ready prerequisites); Foundation (Ready): D-001/D-003. |
...
| [14-performance-foundation](specs/14-performance-foundation.md) | Draft | Five complete canonical JSON performance schemas, workload harness/allocation counter, selected five-warmup-pair/30-measured-pair SHA-256-counter deterministic bootstrap-median algorithm with intermediate goldens, controlled-runner contract, strict 3% boundary, authority, noise, and profile policy. | `10`, `13` (Draft; Ready prerequisites); Foundation (Ready): D-006/D-010. |
| [15-concurrent-corpus](specs/15-concurrent-corpus.md) | Draft | Native parent/exec-child workload with 3 parent and 2 child workers, exact bounded `AF_UNIX SOCK_SEQPACKET` FD-3 wire layouts, one realtime and 32-byte kernel-random call per process, typed run/reliability schemas, 10-second deadline, deterministic result, fault/cleanup, and 100-run gate. No record/replay behavior. | `10`, `13`, `14` (Draft; Ready prerequisites); Foundation (Ready): D-005. |
| [16-security-foundations](specs/16-security-foundations.md) | Draft | Exact pull-request/release Gitleaks v8.30.1 populations, subject-named SHA-256 identities, sole-network acquisition capability, finite M0 resource-limit inventory, CI least privilege, supply-chain/artifact/SBOM integrity, future trust boundaries, and parser/fuzz policy. No unimplemented sandbox claim. | `10`, `11`, `13`, `14`, `15` (Draft; Ready prerequisites); Foundation (Ready): D-007/D-008/D-013/D-015. |
