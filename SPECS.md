...
|---|---|---|---|
| [10-build-environment](specs/10-build-environment.md) | Draft | Nix flake, Bazel/Bzlmod graph, current-stable mutually compatible Clang/LLVM/LLD and GCC set selected into immutable locks, one hash-verifying acquisition profile followed by network-denied actions, exact reference/observed environment schemas, configurations, and subject-named executable/package-tree identities. Does not define target tracing. | Foundation (Ready): D-003/D-004/D-011. |
| [11-governance-release](specs/11-governance-release.md) | Draft | MIT license packaging with exact approved notice `Copyright (c) 2026 Loic Schneider`; outside-contribution closure; exact pin/SPDX normalization; finite evidence/validator/approval inventories; acyclic final-scan/marker order; release provenance; repository/agent policy; and rollback/compatibility claim rules. | `10` (Draft; Ready prerequisite); Foundation (Ready): D-002/D-009/D-016. |
| [12-cli-diagnostics](specs/12-cli-diagnostics.md) | Draft | Minimal `orus --version` and `orus doctor` commands emitting exactly one bounded JSON document per invocation, including usage/contract/renderer failures; exact typed build/component/aggregate check rows, reference-fact paths, exit codes, diagnostics, and false-claim prevention. No help/format/prose or record/replay command surface. | `10`, `11` (Draft; Ready prerequisites); Foundation (Ready): D-001/D-003. |
| [13-ci-quality](specs/13-ci-quality.md) | Draft | GitHub Actions topology for Clang/GCC, dev/release, tests, sanitizers, fuzz smoke, formatting/lint/static, SBOM, corpus, advisory benchmarks, exact evidence/log bounds, immutable cache/secret policy, and sole-network acquisition job with no M0 permission escape. | `10`, `11`, `12` (Draft; Ready prerequisites); Foundation (Ready): D-010. |
| [14-performance-foundation](specs/14-performance-foundation.md) | Draft | Five complete canonical JSON performance schemas and five normative workload records, exact sample ordering/statistics, workload harness/allocation counter, selected five-warmup-pair/30-measured-pair SHA-256-counter deterministic bootstrap-median algorithm with intermediate goldens, controlled-runner contract, baseline-relative strict 3% boundary, authority, noise, and profile policy. | `10`, `13` (Draft; Ready prerequisites); Foundation (Ready): D-006/D-010. |
| [15-concurrent-corpus](specs/15-concurrent-corpus.md) | Draft | Native parent/exec-child workload with 3 parent and 2 child workers, exact bounded `AF_UNIX SOCK_SEQPACKET` FD-3 wire layouts, one realtime and 32-byte kernel-random call per process, typed run/reliability schemas with PID/TID/fault/count invariants, 10-second deadline, deterministic result, cleanup, and 100-run gate. No record/replay behavior. | `10`, `13`, `14` (Draft; Ready prerequisites); Foundation (Ready): D-005. |
| [16-security-foundations](specs/16-security-foundations.md) | Draft | Exact acyclic pull-request/release Gitleaks v8.30.1 populations, subject-named SHA-256 identities, byte-identical sole-network acquisition capability, owner-reconciled finite M0 resource limits, CI least privilege, supply-chain/artifact/SBOM integrity, future trust boundaries, and parser/fuzz policy. No unimplemented sandbox claim. | `10`, `11`, `13`, `14`, `15` (Draft; Ready prerequisites); Foundation (Ready): D-007/D-008/D-013/D-015. |
